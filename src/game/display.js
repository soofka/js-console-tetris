class Display {
  ...
  refresh(matrix, level, score, nextTetrino, messages = []) {
    ...
    const messagesStartRowIndex = Math.floor(matrixSizeY / 2) - Math.floor(messages.length / 2) - 2;
    ...
    matrix.getCells().forEach((row, index) => {
      ...
      if (printMessages && index >= messagesStartRowIndex && index <= messagesEndRowIndex) {
        if (index === messagesStartRowIndex || index === messagesEndRowIndex) {
          rowPrint += new Array((matrixSizeX * 2) + 1).fill('*').join('');
        } else if (index === messagesStartRowIndex + 1 || index === messagesEndRowIndex - 1) {
          rowPrint += `*${new Array((matrixSizeX * 2) - 1).fill(' ').join('')}*`;
        } else {
          const message = messages[index - messagesStartRowIndex - 2];
          rowPrint += `*${[]
            .concat(new Array(Math.floor(((matrixSizeX * 2) - message.length - 1) / 2)).fill(' '))
            .concat(message.split(''))
            .concat(new Array(Math.ceil(((matrixSizeX * 2) - message.length - 1) / 2)).fill(' '))
            .join('')}*`;
        }
      } else {
  ...
  getBoardRow(row) {
    return `|${row.reduce((rowPrint, cell) => `${rowPrint}${cell === 0 ? '_' : cell}|`, '')}`;
    
  }

  getNextTetrinoRow(nextTetrino, rowId) {
    const nextTetrinoTypeId = nextTetrino.getTypeId();
    const nextTetrinoRowCoordinates = nextTetrino.getCoordinates().filter(coordinate => coordinate.y === rowId);

    const nextTetrinoRow = new Array(4).fill(0);
    nextTetrinoRowCoordinates.forEach(coordinate => nextTetrinoRow[coordinate.x] = nextTetrinoTypeId);
