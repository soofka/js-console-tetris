...
class Game {
	...
	init() {
		this.board = new Board(this.config.game.BOARD_WIDTH, this.config.game.BOARD_HEIGHT);
	...
	calculateDelay() {
		...
		const calculateDelayRec = (targetLevel, currentLevel = 1, delay = maxDelay) =>
			currentLevel === targetLevel
				? delay
				: calculateDelayRec(targetLevel, ++currentLevel, parseInt((delay - ((delay - minDelay)/10)), 10));
	...
	updateScoreForFullRows() {
		this.score += this.board.eraseFullRows() * this.levelConfig.SCORE.PER_FULL_ROW;
	}
	...
	gameOver() {
		this.over = true;
		this.stop(['GAME OVER', '', 'PRESS', `[${this.config.controls.PAUSE}]`, 'TO RESTART']);
	}
}
